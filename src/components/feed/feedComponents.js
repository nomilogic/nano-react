...
const Img = function (props) {
  var child = props.children;
  return <Image style={{width: 50, height: 50}} source={{uri: props.src}} />;
};
...
const PostLink = ({item}) => {
  // *ngif="item.type =='link'"
  var urlext = item.mediaUrl.substring(item.mediaUrl.length - 4, item.mediaUrl.length);
        ...
        <View style={{flexDirection: 'row'}}>
          ...
          <View style={{bottom: 2, right: 2}}>
            <View
              ...
              <Text numberOfLines={1} style={{color: Colors.primary}}>View Link</Text>
            </View>
          </View>
        </View>
        {/* *ngif="item.url"           */}
